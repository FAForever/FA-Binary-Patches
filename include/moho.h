...
struct fastvector
{
    ...
    size_t Size() const {
        return sizeof(this->mEnd - this->mStart);
    }
    size_t Capacity() const {
        return sizeof(this->mCapacity - this->mStart);
    }
...
{ // 0x0050DD83, 0x568 bytes
	...
	{
		// all uints are patches to be floats
		float mVisionRadius;
		float mWaterVisionRadius;
		float mRadarRadius;
		float mSonarRadius;
		float mOmniRadius;
		bool mRadarStealth;
		...
		float mRadarStealthFieldRadius;
		float mSonarStealthFieldRadius;
		float mCloakFieldRadius;
...
{
	float mVisionRange;
	float mWaterVisionRange;
	float mRadarRange;
	float mSonarRange;
	float mOmniRange;
	float mRadarStealthRange;
	float mSonarStealthRange;
	float mCloakRange;

	// 0x005BD530
	float GetRange(EEntityAttribute attr) {
		float r;
		asm(
			"call 0x005BD530;"
			: "=a" (r)
...
{ // 0x008B8601, 0x3E8 bytes
	...
	/*virtual*/ bool __thiscall GetWaterIntel(
		/*out*/ float &sonarRange,
		/*out*/ float &waterRange,
		/*out*/ float &radarRange
	);
	/*virtual*/ bool __thiscall GetMaxCounterIntel(/*out*/ float &range);
};
